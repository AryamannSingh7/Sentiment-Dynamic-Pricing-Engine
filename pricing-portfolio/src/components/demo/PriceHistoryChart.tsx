...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    ...
    >
      <p style={{ color: 'var(--ink)', fontWeight: 600, fontSize: 16, marginBottom: 4 }}>
        ${d.price.toFixed(2)}
        <span style={{ fontSize: 11, fontWeight: 500, marginLeft: 8, color: isUp ? 'var(--up)' : 'var(--down)' }}>
          {isUp ? '▲' : '▼'} {d.direction !== 'flat' ? '' : '—'}
        </span>
      </p>
      <p style={{ color: 'var(--muted)', fontSize: 11, marginBottom: 2 }}>{d.time}</p>
      <p style={{ color: 'var(--ink-soft)', fontSize: 11, marginBottom: 4, lineHeight: 1.4, fontFamily: 'var(--font-sans), sans-serif' }}>{d.event}</p>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <span style={{ color: 'var(--muted)', fontSize: 10 }}>Confidence</span>
        <span
          style={{
            fontSize: 10, fontWeight: 600, padding: '1px 6px', borderRadius: 6,
            background: d.confidence >= 0.85 ? 'var(--up-soft)' : d.confidence >= 0.70 ? 'rgba(183,121,31,0.14)' : 'var(--down-soft)',
            color:      d.confidence >= 0.85 ? 'var(--up)' : d.confidence >= 0.70 ? 'var(--warn)' : 'var(--down)',
          }}
        >
...
export default function PriceHistoryChart({ logs, basePrice }: Props) {
  ...
  if (data.length === 0) {
    return (
      <div className="panel h-64 flex flex-col items-center justify-center gap-2" style={{ color: 'var(--text-muted)' }}>
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-3 text-xs" style={{ color: 'var(--text-muted)' }}>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full inline-block" style={{ background: 'var(--up)' }} /> Up
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full inline-block" style={{ background: 'var(--down)' }} /> Down
          </span>
        </div>
      </div>
